...
tape( 'the function returns `false` if provided a nonnegative integer', opts, function test( t ) {
	var i;
	for ( i = 0; i < 101; i++ ) {
		t.strictEqual( isNegativeInteger( i ), false, 'returns expected value when provided '+i );
	}
	t.end();
});
...
tape( 'the function returns `true` if provided a negative integer', opts, function test( t ) {
	var i;
	for ( i = -1; i > -101; i-- ) {
		t.strictEqual( isNegativeInteger( i ), true, 'returns expected value when provided '+i );
	}
	t.end();
});
...
tape( 'the function returns `false` if not provided an integer', opts, function test( t ) {
	...
	for ( i = 0; i < 100; i++ ) {
		v = ( randu()*100.0 ) - 50.0;
		if ( trunc(v) !== v ) {
			t.strictEqual( isNegativeInteger( v ), false, 'returns expected value when provided '+v );
		}
	}
	t.end();
});
...
tape( 'the function returns `false` if provided `NaN`', opts, function test( t ) {
	t.strictEqual( isNegativeInteger( NaN ), false, 'returns expected value' );
	t.end();
});

tape( 'WARNING: the function returns `true` if provided `-infinity`', opts, function test( t ) {
	t.strictEqual( isNegativeInteger( NINF ), true, 'returns expected value' );
	t.end();
});

tape( 'the function returns `false` if provided `+infinity`', opts, function test( t ) {
	t.strictEqual( isNegativeInteger( PINF ), false, 'returns expected value' );
	t.end();
});
